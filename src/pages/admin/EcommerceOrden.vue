    ...
    <q-markup-table>
      ...
      <tbody>
        <tr v-for="(item,index) in ordenes" :key="index">
          ...
          <td class="text-left">
            <q-img :src="item.img"  @click="openURL(item.img)" style="cursor: pointer;"/>
          </td>
          <td class="text-left">
            {{ item.total}}
          </td>
...
<script setup>
  ...
  const refetch = async () => {
    const {data,pagination} =await GetPaginatorOrder(page.value, search.value, initDate.value, endDate.value);
    ordenes.value = data.map((item) => {
      return {
        id: item.id,
        img: item.img,
        total: item.total,
        estado: (item.estado==='pendiente')? false:true,
  ...
  const openURL = (url)=>{
    $q.platform.is.mobile?window.open(url, '_blank'):window.open(url, '_blank')
  }
</script>
