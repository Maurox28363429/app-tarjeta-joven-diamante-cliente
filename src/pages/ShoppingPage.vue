<template>
  <div class="q-pa-lg flex flex-center"><!--Container-->
    ...
    >
      <q-card class="bg-white text-black">
        ...
        <q-card-section>
          ...
          <section>
            <div v-for="item in prouctos" :key="item.id" style="display:inline-flex">
              <q-card class="my-card" flat bordered>
                <img :src="item.img_url ?? 'https://cdn.quasar.dev/img/parallax2.jpg' ">
                <q-list>
                  <q-item>
                    <q-section>
                      <h5>{{item.nombre}}</h5>
                  ...
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{item.descuento}} %</q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
              </q-card>
...
const columns = [
  ...
  {
    name: 'comercio', label: 'Comercio', field: row => row.comercio.name, align: 'left'
  },
  {
    name: 'total', label: 'Total', field: row => row.total, align: 'left'
  },
  {
    name: 'total_descuento', label: 'Descuento total', field: row => row.total_descuento, align: 'left'
  }
]
...
const cargarDatos = async (page = 0) => {
  const queryString = (user.role_id === 2)
  ...
  await instance.get(queryString)
    .then((res) => {
